<template>
  <view>
    <u-navbar
      :title="vuex_nlist[uuid].nickname"
...
export default {
  ...
  async onLoad(opt) {
    ...
    this.uuid = opt.uuid
    this.list = this.vuex_messagelist[this.userinfo.id][this.uuid]
    console.log(this.list)
    await this.$socket.delMessage(this.uuid, false)
    setTimeout(() => {
      uni.pageScrollTo({
        scrollTop: 99999,
        duration: 0
  ...
  methods: {
    submit() {
      ...
      this.$socket.send_message(this.uuid, this.value, 'RC:TxtMsg')
      this.value = ''
    },
    showpopup() {
      this.bottom = '300rpx'
      ...
      this.$u.api
        ...
        .then((res) => {
          ...
          if (res.code == 1) {
            ...
            this.$socket.send_message(this.uuid, param, 'app:hongbao')
            this.sethongbaoshow()

      ...
      >
        ...
        <view class="sendhongbao">
          <view class="item">
            ...
            <view class="content-text" style="right: -105rpx">
              <u-input
                type="number"
                :clearable="false"
                maxlength="3"
            ...
            <view class="foot" style="right: -115rpx">个</view>
          </view>
          <view class="item">
    ...
    >
      <view class="openhongbao">
        ...
        <view class="an">
          ...
          <view
            class="foot"
            v-if="hongbaoinfo.senduser"
            @click="tabhbinfo(hongbaoinfo.hongbaoid)"
            >查看领取详细 ></view
          >
        </view>
      </view>
...
export default {
  ...
  computed: {
    buttomStyle() {
      const style = {}
      if (this.value2 && this.$u.test.amount(this.value2)) {
  ...
  watch: {
    ...
    value2(res1, res2) {
      if (res1 != '') {
        const money = Math.floor(res1 * 100) / 100
        this.value2 = money
        this.value3 = money
      }
    }
  ...
  async onLoad(opt) {
    ...
    await this.$socket.delMessage(this.uuid, true)
    setTimeout(() => {
      uni.pageScrollTo({
        scrollTop: 99999,
        duration: 0
  ...
  methods: {
    submit() {
      ...
      this.$socket.send_message(this.uuid, this.value, 'RC:TxtMsg', true)
      this.value = ''
    },
    showpopup() {
      this.bottom = '300rpx'
    ...
    submithb() {
      ...
      if (!this.$u.test.digits(this.value1)) {
        this.$refs.uTips.show({
          title: '红包个数请输入整数,不能包含小数点',
          type: 'error',
          duration: '2300'
        })
      }
      this.$u.api
        ...
        .then((res) => {
          ...
          if (res.code == 1) {
            ...
            this.$socket.send_message(this.uuid, param, 'app:hongbao', true)
            this.sethongbaoshow()
      ...
      this.$u.api.openisHB({ id: e }).then((res) => {
        ...
        if (res.code == 1) {
          if (res.data.open == 1) {
            ...
            this.hongbaoinfo.senduser = res.data.senduser
          }
        } else {
